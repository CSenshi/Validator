...
def instanceChecker(target_str, result_arr, flag):
    ...
    for _ in r(0, len(result_arr)):
        assert_flag = (assert_flag and (isinstance(
            result_arr[_], R.__all__[target_arr[_]])))
    return assert_flag
...
def test_translator_mid():
    target_arr = [['required'], ['required', 'mail', 'max:30'], [
        'required', 'min:20', 'max:30'], ['required', 'between:10,20']]
    for _ in target_arr:
        target_str = _
        translator = Translator(target_str)
        result_arr = translator.translate()
...
def test_translator_many():
    ...
    # between min
    target_str = "between:" + \
        str(min_value) + ',' + str(max_value) + "|min:" + str(min_value)
    translator = Translator(target_str)
    result_arr = translator.translate()

    assert ((result_arr[0].max_value == max_value)
            and (result_arr[0].min_value == min_value) and((result_arr[1].min_value == min_value)))

    # betweeen max
    target_str = "between:" + \
        str(min_value) + ',' + str(max_value) + "|max:" + str(max_value)
    translator = Translator(target_str)
    result_arr = translator.translate()

    assert ((result_arr[0].max_value == max_value)
            and (result_arr[0].min_value == min_value) and((result_arr[1].max_value == max_value)))

    # betweeen max min
    target_str = "between:" + \
        str(min_value) + ',' + str(max_value) + "|max:" + \
        str(max_value) + "|min:" + str(min_value)
    translator = Translator(target_str)
    result_arr = translator.translate()
